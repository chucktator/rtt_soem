/***************************************************************************
 tag: Ruben Smits  Tue Nov 16 09:31:20 CET 2010  soem_el3064.cpp

 soem_el3064.cpp -  description
 -------------------
...
SoemEL30xx::SoemEL30xx(ec_slavet* mem_loc) :
    soem_master::SoemDriver(mem_loc), m_size(4), m_raw_range(32767), m_lowest(
...

{
    ...
    m_service->addOperation("rawRead", &SoemEL3064::rawRead, this,
            RTT::OwnThread).doc("Read raw value of channel i").arg("i",
            "channel nr");
    m_service->addOperation("read", &SoemEL3064::read, this, RTT::OwnThread).doc(
            "Read value to channel i").arg("i", "channel nr");
    m_service->addOperation("Over_Range", &SoemEL3064::isOverrange, this,
            RTT::OwnThread).doc(
            "For the channel i : 1 = overrange ; 0 = no overrange ").arg("i",
            "channel nr");
    m_service->addOperation("Under_Range", &SoemEL3064::isUnderrange, this,
            RTT::OwnThread).doc(
            "For the channel i : 1 = Underrange ; 0 = no Underrange ").arg("i",
            "channel nr");
    m_service->addOperation("Comp_val_to_lim", &SoemEL3064::checkLimit,
    ...
    m_service->addOperation("Error", &SoemEL3064::is_error, this).doc(
    ...
    // Need to be complete for a specific usage : See Beckhoff : http://www.beckhoff.com/EL3064/
...
void SoemEL3064::update()
{
    m_raw_msg.values[0] = ((out_el3064t*) (m_datap->inputs))->val_ch1;
    m_raw_msg.values[1] = ((out_el3064t*) (m_datap->inputs))->val_ch2;
    m_raw_msg.values[2] = ((out_el3064t*) (m_datap->inputs))->val_ch3;
    m_raw_msg.values[3] = ((out_el3064t*) (m_datap->inputs))->val_ch4;

    m_raw_values_port.write(m_raw_msg);

    m_msg.values[0] = m_raw_msg.values[0] * m_resolution;
    m_msg.values[1] = m_raw_msg.values[1] * m_resolution;
    m_msg.values[2] = m_raw_msg.values[2] * m_resolution;
    m_msg.values[3] = m_raw_msg.values[3] * m_resolution;

    m_values_port.write(m_msg);

    m_params[0] = ((out_el3064t*) (m_datap->inputs))->param_ch1;
    m_params[1] = ((out_el3064t*) (m_datap->inputs))->param_ch2;
    m_params[2] = ((out_el3064t*) (m_datap->inputs))->param_ch3;
    m_params[3] = ((out_el3064t*) (m_datap->inputs))->param_ch4;

}

//rawRead : read the raw value of the input /////////////////////////////////////////////////////
int SoemEL3064::rawRead(unsigned int chan)
{
    if (chan < m_size)
    {
...
double SoemEL3064::read(unsigned int chan)
{
...
bool SoemEL3064::isOverrange(unsigned int chan)
{
    if (chan < m_size)
...
bool SoemEL3064::isUnderrange(unsigned int chan)
{
    if (chan < m_size)
...
bool SoemEL3064::checkLimit(unsigned int chan, unsigned int lim_num)
{
    if (!chan < m_size)
    {
        log(Error) << "Channel " << chan << " is out of the module's range"
    ...
    if (!(lim_num > 0 && lim_num < 5))
    {
        log(Error) << "Limit nr " << lim_num << " should be 1, 2, 3 or 4" << endlog();
        return false;
    }
    if (lim_num == 1)
...
// Checking for errors
bool SoemEL3064::is_error(unsigned int chan)
{
    if (chan < m_size)
...
{
    return new SoemEL3064(mem_loc);
}

const bool registered0 =
        soem_master::SoemDriverFactory::Instance().registerDriver("EL3064",
                createSoemEL3064);
